...
Run:
    ...
    Best results with:
        python SMC_bot_adaptive.py --symbol BTC-USD --period 180d --interval 1h --no-plot --market-mode long_short --entry-touch-mode zone_touch --no-confirmation --min-score 2
"""
    ...
    for touch_idx in range(poi["active_from"] + 1, len(data)):
        ...
        if not cfg.require_confirmation:
            return (
                ...
                f"{cfg.entry_touch_mode}_no_confirmation",
            )
        ...
        for confirm_idx in range(touch_idx, confirm_end):
            if confirmation_candle_ok(
                ...
                return (
                    ...
                    f"{cfg.entry_touch_mode}_confirmed_close_entry",
                )

        return None, None, f"{cfg.entry_touch_mode}_touch_without_confirmation"
...
def backtest(
    ...
    for poi in pois:
        ...
        if direction == 1:
            ...
            if risk_per_unit <= 0:
                continue
            target = entry + cfg.rr * risk_per_unit
        else:
            stop = top + buffer
...
def main():
    ...
    parser.add_argument("--no-confirmation", action="store_true", help="Matikan confirmation")
